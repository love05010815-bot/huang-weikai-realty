/* ⛔ 已停用（2026-08-23）——— 這支檔案沒有任何地方 import 了。
 *
 * 原本的作法：拿精選好案的 `area` 欄（「行政區・建案名」）做文字比對，
 * 把物件掛到 /map 的建案底下。系統擁有者拍板改掉，理由有兩個：
 *   ① 打錯字物件就默默從地圖消失，不會報錯，很難發現
 *   ② 沒辦法「只上架到地圖、不上架到首頁」——— 兩邊綁死
 *
 * 現在改成獨立的一套：`src/lib/map-listings.ts` ＋ 後台 `/admin/map-listings`，
 * 每筆物件在後台明確指定屬於哪個建案。
 *
 * 留著這支檔案只是備查，**不要再接回去**。 */
/**
