"use server";
/**
 * 同事授權碼後台（/admin/post591/keys）的動作：新增、停用／恢復、解除綁定、改到期日、刪除。
 *
...
import { createLicense, deleteLicense, setLicenseExpires, setLicenseRevoked, taiwanDateEnd, unbindLicense } from "@/lib/ext-license";
...
export async function createLicenseAction(name: string, expiresDate: string): Promise<Result & { key?: string }> {
  if (!(await isCurrentUserAdmin())) return { ok: false, error: "權限不足" };
  const expiresAt = taiwanDateEnd(expiresDate);
  if (!expiresAt) return { ok: false, error: "到期日格式不對（要 YYYY-MM-DD）" };
  try {
    const row = await createLicense(name, expiresAt);
    revalidatePath(PAGE);
...
export async function unbindLicenseAction(id: string): Promise<Result> {
  return guarded(() => unbindLicense(id));
}
