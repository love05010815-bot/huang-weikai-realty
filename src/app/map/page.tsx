 ...
 *    ⚠️ **2026-08-31 第二次拍板：`<title>` 改成四個地名打頭**（梧棲・清水・沙鹿・鹿寮），
 ...
 *
 * ⚠️ **2026-08-31 系統擁有者再次拍板：標題改成四個地名打頭 —— 梧棲・清水・沙鹿・鹿寮。**
 * 「台中港市鎮中心」從 `<title>` 拿掉了（前一版刻意把它擺最前面保排名，這一版不是）。
 *
 * 為什麼可以拿掉：那是重劃區的正式名稱，**一般買方不會這樣搜**；
 * 客戶真的打進 Google 的是行政區名（梧棲／清水／沙鹿）跟在地說法（鹿寮）。
 * 而且它沒有從這頁消失 —— 內文出現 17 次，keywords、description、JSON-LD 都還在，
 * **拿掉的只有 `<title>` 那一行**。
 *
 * ⚠️ 四個地名都對得起案數：梧棲＋清水＝重劃區 39 案、鹿寮萬家福 66 案、沙鹿車站 28 案，
 *    合計 133 案。**不要再往裡面加地名**，理由見下一段。
 *
 ...
 *    不把沙鹿寫進 H1 是同一條。
 */
const TITLE = `梧棲・清水・沙鹿・鹿寮建案地圖｜台中海線 ${stats.total} 案一次看｜房仲${OWNER.name}`;
