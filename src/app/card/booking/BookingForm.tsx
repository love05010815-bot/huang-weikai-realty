...
function qualificationFields(mode: BookingMode, intent: string) {
  if (mode === "collaboration") {
    return [
      ...
      { key: "purpose", label: "希望合作的內容", placeholder: "請說明形式、對象與希望瑋凱參與的部分", required: true, multiline: true },
  ...
  if (intent === "buy") {
    return [
      { key: "area", label: "希望區域", placeholder: "例：沙鹿、梧棲、清水、龍井", required: true },
      { key: "budget", label: "購屋預算", placeholder: "例：1,000 萬", required: true },
      { key: "purpose", label: "需求重點", placeholder: "例：自住三房、有車位、近學區", required: true, multiline: true },
      { key: "targetDate", label: "預計購屋時間", placeholder: "例：3 個月內", required: true },
    ];
  }
  if (intent === "sell") {
    return [
      { key: "propertyAddress", label: "物件大約位置", placeholder: "例：沙鹿區範例路，填到路名即可", required: true },
  ...
  if (intent === "tax") {
    return [
      { key: "legalTopic", label: "問題類型", placeholder: "例：房地合一稅、重購退稅、持有稅務", required: true },
      { key: "purpose", label: "事情摘要", placeholder: "請簡述物件狀況與想了解的稅務問題", required: true, multiline: true },
      { key: "targetDate", label: "是否有期限", placeholder: "例：申報期限前；目前沒有", required: true },
    ];
  }
  if (intent === "reno") {
    return [
      ...
      { key: "purpose", label: "想整理的重點", placeholder: "例：交屋前簡單翻新、想要的風格", required: true, multiline: true },
      { key: "targetDate", label: "希望完成時間", placeholder: "例：交屋前、3 個月內", required: true },
    ];
  }
  return [
...
export default function BookingForm() {
  ...
  const validate = (): FieldErrors => {
    ...
    if (meetType === "custom" && !customReady) next.meetType = "指定地點必須使用瑋凱核准後提供的專屬連結。";
      ...
      <main className={styles.page}>
        <div className={styles.shell}>
          <header className={styles.header}>
            <div className={styles.brand}>太平洋房屋・黃瑋凱</div>
  ...
  return (
    <main className={styles.page}>
      <div className={styles.shell}>
        <header className={styles.header}>
          <div className={styles.brand}>太平洋房屋・黃瑋凱</div>
          <h1 className={styles.title}>預約與瑋凱聊聊</h1>
          ...
          {bookingMode && intentKey ? (
            <section className={styles.section}>
              ...
              {approvalState === "approved" && !approvedLocation ? (
                <div className={styles.errorNotice}>核准連結缺少已同意的地點，無法開放指定地點。請聯絡瑋凱重新產生連結。</div>
              ) : null}
              {approvalState === "invalid" ? (
                <div className={styles.errorNotice}>指定地點核准連結已失效、已使用或被撤銷，請重新聯絡瑋凱。</div>
              ) : null}
              ...
              {!customReady && approvalState !== "checking" ? (
                <div className={styles.inlineNotice}>
                  需要指定其他地點？請先
                  {" "}<a className={styles.link} href={SOCIAL.line} target="_blank" rel="noopener noreferrer">加瑋凱私人 LINE</a>
                  {" "}確認。取得專屬連結後，系統會直接帶入核准地點。
                </div>
              ) : null}
              {meetType === "custom" && approvedLocation ? (
                <div className={styles.field}>
                  <span className={styles.label}>瑋凱已核准的指定地點</span>
                  ...
                  <p className={styles.sectionHint}>此地點已鎖定，若要更換請先與瑋凱重新確認。</p>
                </div>
              ) : null}
            </section>
          ) : null}
              ...
              <div ref={registerRef("slot")} tabIndex={-1}>
                ...
                {!loadingSlots && !slotError && days.length === 0 ? (
                  <div className={styles.inlineNotice}>
                    ...
                    {" "}<a className={styles.link} href={SOCIAL.line} target="_blank" rel="noopener noreferrer">直接聯絡瑋凱</a>。
                  </div>
                ) : null}
                {days.length ? (
                  <>
