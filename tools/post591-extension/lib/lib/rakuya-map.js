 ...
 * 他 2026-09-10 的預設（沒回就照這樣）：聯絡人自行填寫「」、委託到期日留空、有管理費 → 管理方式「管理員(警衛)」、
...
export function rakuyaDesc(desc) {
...
export function rakuyaParkKind(parkType) {
    ...
    if (/坡道.*平面|平面.*坡道/.test(p))
        return "坡道平面式";
    if (/坡道.*機械|機械.*坡道/.test(p))
        return "坡道機械式";
    if (/(升降|昇降).*平面|平面.*(升降|昇降)/.test(p))
        return "昇降平面式";
    if (/(升降|昇降).*機械|機械.*(升降|昇降)/.test(p))
        return "昇降機械式";
    if (/循環/.test(p))
        return "機械循環式";
    if (/庭院/.test(p))
...
export function buildRakuya(d, o, p, nowYear = new Date().getFullYear()) {
    ...
    if (rent && d.rentCond.parkIncluded && !includesRakuya.includes("停車費"))
        includesRakuya.push("停車費");
