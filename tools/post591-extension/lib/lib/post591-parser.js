 ...
 */
export function halfWidthDigits(s) {
 ...
 */
export function isHouseolCatalogHtml(html) {
    if (!/class="t-t[dh]"/.test(html))
        return false;
 ...
 */
export function houseolHtmlToText(html) {
    const out = ["不動產電子型錄"];
    ...
    if (agent)
        out.push(`經紀人員：${agent[1].trim()}`);
    const footer = html.match(/<div id="footer"[^>]*>([\s\S]*?)<\/div>/);
    if (footer)
        out.push(stripTags(footer[1]).replace(/\n/g, " "));
    return out.join("\n");
}
/**
