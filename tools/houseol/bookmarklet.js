 ...
 *   2. 把目前這一頁的每一列轉成一筆資料
 *   3. 從門牌地址抽出行政區（例如「梧棲區」），**門牌本身也留著**
 *   4. 下載的 json 因此含完整門牌 —— 那個檔是本機檔，不進版控也不要外傳
 *   5. 下載成一個 json 檔（瀏覽器的「下載項目」資料夾）
 *
 * 111 筆分 12 頁，所以要「翻頁 → 點書籤 → 翻頁 → 點書籤…」重複 12 次，
 * 抓完把所有下載下來的 json 檔丟給 import.js 合併。
 *
 * ⚠️ **門牌地址絕對不能進 `src/config/houseol-inventory.json`** —— 那個檔有版控、
 *    repo 是公開的。剔除的工作在 `sanitize.js`，地址另外走
 *    `addresses.local.json` →（`push-addresses.js`）→ 資料庫這條路。
 *
...
(function () {
  ...
  for (var r = 1; r < table.rows.length; r++) {
    ...
    // 門牌地址留著 —— 後台「從愛屋帶入」要用它填物件地址欄。
    // 剔除的工作交給 sanitize.js（庫存檔那條路），這裡不要先砍掉，
    // 砍掉的話地址就再也回不來了，只能重抓一次。
  ...
  alert(
    "抓到 " + rows.length + " 筆，已下載成一個 json 檔（在瀏覽器的「下載項目」資料夾）。\n\n" +
      ...
      "⚠️ 這些檔案含完整門牌地址，不要外傳、不要丟進 GitHub。"
  );
})();
